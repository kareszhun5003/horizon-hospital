	    ...
	    <div class="row">
	    	...
	    	<div class="col-md-3 left-column">
	    		...
	    		<div class="chatter_sidebar">
					<button style="color: white;" class="btn btn-primary" id="new_discussion_btn"><i class="chatter-new"></i> @lang('chatter::messages.discussion.new')</button>
					<a style="color: white;" href="/{{ Config::get('chatter.routes.home') }}"><i class="chatter-bubble"></i> @lang('chatter::messages.discussion.all')</a>
          {!! $categoriesMenu !!}
				</div>
				<!-- END SIDEBAR -->
	    	</div>
	        <div class="col-md-9 right-column">
	        	<div class="panel">
		        	<ul class="discussions inv-div" style="background-color: rgba(255, 0, 0, 0);">
		        		@foreach($discussions as $discussion)
				        	<li>
				        		<a style="background-color: rgba(255, 0, 0, 0);" class="discussion_list inv-div" href="/{{ Config::get('chatter.routes.home') }}/{{ Config::get('chatter.routes.discussion') }}/{{ $discussion->category->slug }}/{{ $discussion->slug }}">
					        		<div class="chatter_avatar">
					        			...
					        			@else
					        				...
					        				<span>
					        					<img class="rounded-circle" src="{{ URL::to('/') }}/img/uploads/avatars/{{ $discussion->user->avatar }}">
					        				</span>
                        ...
                        @if($discussion->user->type == 'doctor')
                          <h3 class="chatter_middle_title text-danger">{{ $discussion->title }} <div class="chatter_cat" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</div></h3>
                        @elseif($discussion->user->type == 'admin')
                          <h3 class="chatter_middle_title text-warning">{{ $discussion->title }} <div class="chatter_cat" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</div></h3>
                        @else
                          <h3 class="chatter_middle_title text-light">{{ $discussion->title }} <div class="chatter_cat" style="background-color:{{ $discussion->category->color }}">{{ $discussion->category->name }}</div></h3>
                        @endif
                        ...
                        <span class="chatter_middle_details">@lang('chatter::messages.discussion.posted_by') <span data-href="/user">{{ ucfirst($discussion->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</span> {{ \Carbon\Carbon::createFromTimeStamp(strtotime($discussion->created_at))->diffForHumans() }}</span>
					        			...
					        			@endif
					        			<p class="text-light">{{ substr(strip_tags($discussion_body), 0, 200) }}@if(strlen(strip_tags($discussion_body)) > 200){{ '...' }}@endif</p>
					        		</div>
