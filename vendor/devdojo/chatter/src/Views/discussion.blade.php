...
<div id="chatter" class="discussion" style="display: inline">
	...
	<div class="container margin-top">
	    ...
	    <div class="row">
            ...
            @endif
				...
				<div class="conversation text-break">
	                <ul class="discussions no-bg">
	                	@foreach($posts as $post)
	                		<li data-id="{{ $post->id }}" data-markdown="{{ $post->markdown }}">
		                		<span class="chatter_posts" style="background-color: rgba(255, 0, 0, 0);">
                        @if(Auth::guest())
                        @elseif(!Auth::guest() && (Auth::user()->id == $post->user->id))
			                			...
			                			<div class="chatter_post_actions">
			                				<p class="chatter_delete_btn text-dark">
			                					<i class="chatter-delete"></i> @lang('chatter::messages.words.delete')
			                				</p>
			                				<p class="chatter_edit_btn text-dark">
                          ...
                          <div class="chatter_avatar">
					        			...
					        			@else
                          <span>
					        					<img class="rounded-circle" src="{{ URL::to('/') }}/img/uploads/avatars/{{ $post->user->avatar }}">
					        				</span>
					        			@endif
					        		</div>
                        ...
                        @if($post->user->type == 'doctor')
                          <a class="text-danger" href="{{ \DevDojo\Chatter\Helpers\ChatterHelper::userLink($post->user) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span class="ago chatter_middle_details text-light">
                            <br>
                            {{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
                        @elseif($post->user->type == 'admin')
                          <a class="text-warning" href="{{ \DevDojo\Chatter\Helpers\ChatterHelper::userLink($post->user) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span class="ago chatter_middle_details text-light">
                            <br>
                            {{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
                        @else
                          <a class="text-light" href="{{ \DevDojo\Chatter\Helpers\ChatterHelper::userLink($post->user) }}">{{ ucfirst($post->user->{Config::get('chatter.user.database_field_with_user_name')}) }}</a> <span class="ago chatter_middle_details text-light">
                            <br>
                            {{ \Carbon\Carbon::createFromTimeStamp(strtotime($post->created_at))->diffForHumans() }}</span></span>
                        @endif
	            ...
	            @if(!Auth::guest())
                  ...
                  <div class="chatter_avatar">
		        			...
		        			@else
		        				<span> <img  src="/img/uploads/avatars/{{Auth::user()->avatar}}">
		        				</span>
		        			@endif
		        		</div>
						...
						<div id="discussion_response_email">
							<button id="submit_response" class="btn btn-light pull-right"><i class="chatter-new"></i> @lang('chatter::messages.response.submit')</button>
...
<script>
	$('document').ready(function(){
		...
		$('.chatter_edit_btn').click(function(){
			...
			container.append('<div class="chatter_update_actions"><button class="btn btn-dark pull-right update_chatter_edit"  data-id="' + id + '" data-markdown="' + markdown + '"><i class="chatter-check"></i> @lang('chatter::messages.response.update')</button><button href="/" class="btn btn-default pull-right cancel_chatter_edit" data-id="' + id + '"  data-markdown="' + markdown + '">@lang('chatter::messages.words.cancel')</button></div>');
